...
@Component({
  selector: 'app-show-release-calender-months-series',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './show-release-calender-months-series.html',
  styleUrl: './show-release-calender-months-series.scss',
})
export class ShowReleaseCalenderMonthsSeries implements OnInit, OnDestroy {
  public currentYear: number = new Date().getFullYear();

  public months: ReleaseMonthItem[] = [];

  public top10Series: TmdbSearchMultiResult[] = [];

  public readonly fallbackPosterSrc: string = '/assets/Missingposter/missingposter.png';

  private posterSubscriptions: Subscription[] = [];
  private topSeriesSubscription: Subscription | null = null;
  ...
  public ngOnDestroy(): void {
    ...
    if (this.topSeriesSubscription) {
      this.topSeriesSubscription.unsubscribe();
      this.topSeriesSubscription = null;
    }
  }
  ...
  public getTopSeriesPosterSrc(series: TmdbSearchMultiResult): string {
    if (typeof series.poster_path === 'string' && series.poster_path.trim().length > 0) {
      return this.tmdbService.getPosterUrl(series.poster_path, 'w500');
    }

    return this.fallbackPosterSrc;
  }

  public getTopSeriesPosterAlt(series: TmdbSearchMultiResult): string {
    const title = this.tmdbService.getDisplayTitle(series);

    if (typeof title === 'string' && title.trim().length > 0) {
      return title;
    }

    return 'Missing series poster';
  }

  public getTopSeriesReleaseDate(series: TmdbSearchMultiResult): string {
    ...
    return 'First air date unavailable';
  }
  ...
  private loadTop10SeriesForYear(): void {
    const startDate = this.formatDate(this.currentYear, 1, 1);
    const endDate = this.formatDate(this.currentYear, 12, 31);

    this.topSeriesSubscription = this.tmdbService.discoverTvAdvanced({
      page: 1,
      language: 'en-US',
      sortBy: 'popularity.desc',
      firstAirDateGte: startDate,
      firstAirDateLte: endDate
    }).subscribe({
      next: (response: BackendTmdbResponse<TmdbSearchMultiResponse>) => {
        ...
        const validResults = responseResults.filter((item: TmdbSearchMultiResult) => {
          ...
          if (!firstAirDate.startsWith(`${this.currentYear}-`)) {
            return false;
          }
        ...
        this.top10Series = validResults.slice(0, 10);
        this.cdr.detectChanges();
      },
      error: () => {
        this.top10Series = [];
  ...
  private loadRandomPosterForMonth(month: ReleaseMonthItem): void {
    const subscription = this.tmdbService.discoverTvAdvanced({
      page: 1,
      language: 'en-US',
      sortBy: 'popularity.desc',
      firstAirDateGte: month.startDate,
      firstAirDateLte: month.endDate
    }).subscribe({
      next: (response: BackendTmdbResponse<TmdbSearchMultiResponse>) => {
        const responseResults = Array.isArray(response?.data?.results) ? response.data.results : [];
        ...
        const validResults = responseResults.filter((item: TmdbSearchMultiResult) => {
          ...
          if (!firstAirDate.startsWith(`${month.year}-${String(month.monthNumber).padStart(2, '0')}`)) {
            return false;
          }
        ...
        const randomSeries = validResults[randomIndex];

        month.posterPath = randomSeries.poster_path || null;
        month.posterTitle = this.tmdbService.getDisplayTitle(randomSeries);
  ...
  public trackByTopSeriesId(index: number, item: TmdbSearchMultiResult): number {
    return item.id;
  }
}